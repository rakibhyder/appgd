...
test_dataset='CelebA'
z_dim=256
sub_dim_pool=[5000]#500,1000.1500,2000,2500,3000,3500,4000
sub_dim_mse=[]
sub_dim_psnr=[]
sub_dim_ssim=[]
sub_dim_rec=[]
...
#                    np.random.seed(seed)
            ...
            #            optimizer.step()
                ...
                if np.mean((x_true-x)**2)<np.mean((x_true-x_best)**2) or random_restart==1:
                    ...
                    x_rec1[epoch_idx[batch_idx*test_batch_size:np.min([(batch_idx+1)*test_batch_size,test_size])],:]=x.reshape(x.shape[0],nc,ngf,ndf)
...
#                z_test=z_test_temp
    ...
    #            plt.show()
        ...
        np.savez('/media/rakib/Data/Data/GLO/icassp2019/Results/'+train_dataset+'_'+test_z_type+'_'+str(z_dim)+z_norm_type+'_alpha'+str(test_alpha)+'_epochs'+str(test_epochs)+'_'+str(test_batch_size)+'phase_retrieval_init_'+x_init+subtype+'_subsample_'+str(sub_dim),z_test=z_test,mask=mask,x_rec=x_rec,x_rec_org=x_rec_org)
