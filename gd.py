...
#                    np.random.seed(seed)
                ...
                for epoch in range (0,test_epochs):
                    ...
                    loss=(x_hat_mask - x_batch_tensor).pow(2).mean()#sub_dim*
...
#            loss_test_outer.append(np.array(loss_outer_epoch_best))
    ...
    #            plt.show()
        ...
        np.savez('/media/rakib/Data/Data/GLO/icassp2019/Results/'+train_dataset+'_'+test_z_type+'_'+str(z_dim)+z_norm_type+'_alpha'+str(test_alpha)+'_epochs'+str(test_epochs)+'_'+str(test_batch_size)+'phase_retrieval_direct_'+subtype+'_subsample_'+str(sub_dim),z_test=z_test,mask=mask,x_rec=x_rec)
